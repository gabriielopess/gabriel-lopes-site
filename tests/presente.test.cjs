...
global.fetch = async (_, options) => {
  ...
  else if (command === 'HDEL') result = rows.delete(args[1]) ? 1 : 0;
...
test('private responses, validated writes, retries, session and outages', async () => {
  ...
  const second = { ...data, id: '7b9c97dc-5c22-4d5a-b6bc-211ec89cf481', name: 'Outra pessoa' };
  await call('submit', second);
  assert.equal((await call('delete', { id: data.id })).code, 401);
  assert.equal(rows.size, 2);
  assert.equal((await call('delete', { id: data.id }, cookie, { origin: 'https://evil.test' })).code, 403);
  assert.equal((await call('delete', { id: 'bad' }, cookie)).code, 400);
  offline = true;
  assert.equal((await call('delete', { id: data.id }, cookie)).code, 503);
  assert.equal(rows.size, 2);
  offline = false;
  assert.equal((await call('delete', { id: data.id }, cookie)).code, 200);
  assert.equal(rows.has(data.id), false);
  assert.equal(rows.has(second.id), true);
  assert.equal((await call('delete', { id: data.id }, cookie)).code, 200);
